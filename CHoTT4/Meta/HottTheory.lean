...
namespace CHoTT4.Meta.HottTheory
...
def hasLargeElim (type : Name) : MetaM Bool := do {
  let typeFormerIsProp ← Meta.forallTelescopeReducing (← typeOf type) (λ _ e => return Expr.isProp e);
  let elimIsProp ← Meta.forallTelescopeReducing (← typeOf (type ++ `rec)) (λ _ e => isProof e);
...
def defHoTT (_ declMods declId declDef : Syntax) : CommandElabM Name := do {
